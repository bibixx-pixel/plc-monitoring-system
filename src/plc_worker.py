...
class Worker(QThread):
	...
	def __init__(self, ip, port):
		super().__init__()
		# 스레드의 동작 상태(ON/OFF)를 제어하기 위해 만든 플래그 변수 (스위치 역할함)
		self.power = True
		self.ip = ip
		self.port = port
	...
	def run(self):
		...
		# self.power가 True인 동안(프로그램 실행 중) 계속 재연결 시도
		while self.power:
			...
			# 에러가 발생하여 수집이 중단되었을 경우 문구 출력, 에러의 구체적인 이유를 ex 변수에 자동으로 담아줌.
				...
				# 사용자가 직접 해제 버튼을 누른 게 아니라면 에러 로그 출력
				# (사용자 직접 해제 버튼 에러 로그는 'def communication_disconnect()'에 따로 해놨음.)
				if self.power:
			...
			# 소켓 닫고 3초 뒤에 바깥 while문으로 올라가서 재접속 시도
			finally:
				client_socket.close()

				# 사용자가 해제 버튼을 누른게 아니라 통신 장애라면 3초 대기 후 재연결하기
				if self.power:
						time.sleep(3)
